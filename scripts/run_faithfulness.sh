...
do
    ...
    CUDA_LAUNCH_BLOCKING=1 python3 ${BASE_PATH}/faithfulness/faithfulness.py --metric "${METRIC}" --model_name "${MODEL_NAME}" --model_type "${MODEL_NAME}" --dataset "news_qa"
    CUDA_LAUNCH_BLOCKING=1 python3 ${BASE_PATH}/faithfulness/faithfulness.py --metric "${METRIC}" --model_name "${MODEL_NAME}" --model_type "${MODEL_NAME}" --dataset "natural_questions"
    CUDA_LAUNCH_BLOCKING=1 python3 ${BASE_PATH}/faithfulness/faithfulness.py --metric "${METRIC}" --model_name "${MODEL_NAME}" --model_type "${MODEL_NAME}" --dataset "bioasq"
