...
def get_raw_scores(examples, preds):
    ...
    for idx in range(len(examples["id"])):
        qas_id = examples["id"][idx][0]
        # print(examples["answers"][idx])
        answers = examples["answers"][idx][0]["text"]
        print(answers)
        gold_answers = [answer for answer in answers if normalize_answer(answer)]
        print(gold_answers)
        ...
        if qas_id not in preds["id"]:
            print("Missing prediction for %s" % qas_id)
            continue

        prediction = preds["answers"][idx][0]["answer"]
...
def apply_no_ans_threshold(scores, na_probs, qid_to_has_ans, na_prob_thresh):
    ...
    for qid, s in scores.items():
        pred_na = int(na_probs["answers"]) > na_prob_thresh
        if pred_na:
