"""
File to run the lime interpretability
"""
...
from utils import run_lime_attribution
...
class LimeLM:
    ...
    def run_lime(self, features, answers):
        ...
        np_attribution = run_lime_attribution(len(tokens), score_fn).reshape((1, -1))
        return torch.from_numpy(np_attribution)

    def lime_interp(self, prefix=""):
        args = self.args
        ...
        dataset = dataset.select(range(535, 2500))
        ...
        for idx, batch in tqdm(enumerate(eval_dataloader), desc="Interpreting",
            ...
            if idx == args.first_n_samples:
                break
            # print(batch)
            # if batch["id"][0] == "6b67d06dbf4b4e2eae802b3e341ce8d4":
            ...
            with torch.no_grad():
                importances = self.run_lime(features, answers)
            ...
            self.dump_lime_info(args, batch, features, answers, importances)
    ...
    def dump_lime_info(self, args, examples, features, predictions, attributions):
        """
        Save the LIME attributions to a file.
        """
...
def main():
    ...
    if args.do_vis:
        ...
        interpret = LimeLM(args, model, tokenizer)
        # Evaluate
        result = interpret.lime_interp()
