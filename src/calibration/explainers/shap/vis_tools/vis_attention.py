...
class LinkGraph:
        ...
        for i in range(num_tokens):
            for j in range(num_tokens):
                ...
                if abs(val) < PLOT_THRESHOLD:  # 1/100 basically transparent
                    continue
                self.links.append(
                    TextLink(self.left_texts[i], self.right_texts[j], val)
                )

        self.title_text = "num: {}, sum: {:.3f}, max_attr: {:.3f}, min_attr: {:.3f}".format(
            len(self.links), sum_attr, max_attr, min_attr
        )
    ...
    def _arrange_links(self, dummy_draw, font):
        for i, link in enumerate(self.links):
            ...
            link.x0, link.y0 = (
                GLOBAL_W_PADDING + TEXT_W_PADDING,
                GLOBAL_H_PADDING + (src.index + 0.5) * TEXT_HEIGHT,
            )
            link.x1, link.y1 = (
                GLOBAL_W_PADDING + TEXT_GAP - TEXT_W_PADDING,
                GLOBAL_H_PADDING + (dst.index + 0.5) * TEXT_HEIGHT,
            )
    ...
    def render(self, draw, font):
        ...
        for l in sorted(self.links, key=lambda x: abs(x.val)):
            if (l.val >= 0 and self.vis_positive) or (l.val < 0 and self.vis_negative):
                draw.line(
                    ((l.x0, l.y0), (l.x1, l.y1)),
                    fill=positive_color(l.val)
                    if l.val >= 0
                    else negative_color(-l.val),
                    width=2,
                )


def visualize_connection(
    filename, tokens, connection, interp_info, vis_positive=True, vis_negative=True
):
