    ...
    def compute_scores(self):
        ...
        with jsonlines.open(os.path.join(f"{BASE_PATH}src/data/squad/", self.data_path)) as reader:
            for example in tqdm(reader):
                ...
                orig_example = [sample for sample in self.squad_data if sample["id"] == id][0]
                orig_question = orig_example["question"]
                # print(cf_question, orig_question)
                if self.metric == "self_bleu":
                    sim_score = self.calculate_self_bleu([orig_question], cf_question)["bleu4"]
                elif self.metric == "sbert_sim":
                    sim_score = self.calculate_semantic_similarity(orig_question, cf_question)
                self.scores.append(sim_score)
                results.append(
                    {"id": id, "sentence1": orig_question, "sentence2": cf_question, "similarity_score": float(sim_score)}
                )
            # average scores
    ...
    def compute_scores_batched(self):
        ...
        with jsonlines.open(os.path.join(f"{BASE_PATH}src/data/squad/", self.data_path)) as reader:
            ...
            for example in tqdm(reader):
                ...
                sentences2.append(
                    squad_data_map.get(orig_example_id, ""))  # Use get() to handle cases where "id" not found
        ...
        for i, (id, sentence1, sentence2, sim_score) in enumerate(zip(idx, sentences1, sentences2, similarity_scores)):
