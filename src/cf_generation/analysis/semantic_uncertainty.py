...
class SemanticSimilarity:
    ...
    def _load_data(self, squad_data_map):
        ...
        sentences2 = [
            squad_data_map.get(example["id"].split("_")[0], "") for example in data
        ]
    ...
    def compute_entailment(self):
        ...
        for batch in tqdm(dataloader):
            ...
            padded_inputs = padded_inputs.to(
                self.device
            )  # Ensure tensor type and conversion
            padded_reverse_inputs = padded_reverse_inputs.to(
                self.device
            )  # Ensure tensor type and conversion
            ...
            deberta_predictions_batch = (predicted_labels != 0) & (
                reverse_predicted_labels != 0
            )
            deberta_predictions.extend(deberta_predictions_batch.tolist())
            # print(deberta_predictions)

            forward_entailment_probs_batch = torch.softmax(prediction, dim=1)[
                :, 2
            ].tolist()  # Probability of entailment
            reverse_entailment_probs_batch = torch.softmax(reverse_prediction, dim=1)[
                :, 2
            ].tolist()
            ...
            for i in range(len(qa_1)):
                ...
                reverse_deberta_pred = (
                    reverse_predicted_labels[i] != 0
                )  # Class 2 is Entailment
        ...
        if forward_entailment_probs and reverse_entailment_probs:
            avg_forward_entailment_prob = sum(forward_entailment_probs) / len(
                forward_entailment_probs
            )
            avg_reverse_entailment_prob = sum(reverse_entailment_probs) / len(
                reverse_entailment_probs
            )
            print(
                f"Average Forward Entailment Probability for True samples: {avg_forward_entailment_prob:.4f}"
            )
            print(
                f"Average Reverse Entailment Probability for True samples: {avg_reverse_entailment_prob:.4f}"
            )

            # Calculate and print the overall average of both directions
            overall_avg_entailment_prob = (
                avg_forward_entailment_prob + avg_reverse_entailment_prob
            ) / 2
            print(
                f"Overall Average Entailment Probability: {overall_avg_entailment_prob:.4f}"
            )
        else:
