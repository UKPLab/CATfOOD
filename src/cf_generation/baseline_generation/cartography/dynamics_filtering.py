...
def compute_train_dy_metrics(training_dynamics, args):
    ...
    if (
        ...
        variability_func = lambda conf: np.sqrt(
    ...
    try:
        for guid in tqdm(training_dynamics):
            ...
            for i, (start_epoch_logits, end_epoch_logits) in enumerate(
                zip(record["start_logits"], record["end_logits"])
            ):
                start_probs = torch.nn.functional.softmax(
                    torch.Tensor(start_epoch_logits), dim=-1
                )
                # print(start_probs)
                end_probs = torch.nn.functional.softmax(
                    torch.Tensor(end_epoch_logits), dim=-1
                )
                # print(end_probs)
                ...
                is_correct = (start_prediction == record["start_gold"]).item() & (
                    end_prediction == record["end_gold"]
                ).item()
            ...
            confidence_[guid] = (
                np.sum(np.mean(true_start_probs_trend) + np.mean(true_end_probs_trend))
                / 2
            )
            variability_[guid] = (
                np.sum(
                    variability_func(true_start_probs_trend)
                    + variability_func(true_end_probs_trend)
                )
                / 2
            )
    ...
    def compute_loss(idx: int):
        ...
        combined_loss = (
            loss(
                torch.tensor(start_logits[idx]), torch.LongTensor(start_targets[idx])
            ).item()
            + loss(
                torch.tensor(end_logits[idx]), torch.LongTensor(end_targets[idx])
            ).item()
        )
        combined_loss /= 2
    ...
    df_train = pd.DataFrame(
        [
            [i, compute_loss(i), training_accuracy[i] / len(training_dynamics)]
...
def write_filtered_data(args, train_dy_metrics):
    ...
    with open(
        os.path.join(args.filtering_output_dir, f"filtering_configs.json"), "w"
    ) as outfile:
    ...
    # Sort by selection.
    sorted_scores = train_dy_metrics.sort_values(
        by=[args.metric], ascending=is_ascending
    )
    ...
    dataloader = PreprocessData(
        ...
        cf_path=os.path.join(
            args.data_dir,
            "rag_counterfactuals_complete_noise_min_filtered_final_dedup_1.jsonl",
        ),
        save_data=False,
        save_path="",
    )
    ...
    for fraction in [0.01, 0.05, 0.10, 0.1667, 0.25, 0.3319, 0.50, 0.75]:
        ...
        with jsonlines.open(os.path.join(outdir, f"train.jsonl"), "w") as writer:
            for example in tqdm(
                train_data, total=len(train_data), desc="Saving samples ... "
            ):
                # print(example["id"])
...
def plot_data_map(
    ...
    dataframe = dataframe.sample(
        n=max_instances_to_plot
        if dataframe.shape[0] > max_instances_to_plot
        else len(dataframe)
    )
    ...
    filename = (
        ...
        if show_hist
        else f"figures/compact_{title}_{model}.pdf"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model", default="RoBERTa", help="Model for which data map is being plotted"
    )
    parser.add_argument(
        "--dataset_name",
        default="squad",
