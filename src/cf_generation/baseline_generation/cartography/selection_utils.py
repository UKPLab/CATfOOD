...
def read_training_dynamics(
  ...
  """
    ...
    num_epochs = len(
        [f for f in os.listdir(td_dir) if os.path.isfile(os.path.join(td_dir, f))]
    )
    if burn_out:
        num_epochs = burn_out
        ...
        with open(epoch_file, "r") as infile:
            for line in infile:
                ...
                if guid not in train_dynamics:
                    assert epoch_num == 0
                    train_dynamics[guid] = {
                        "start_gold": record["start_gold"],
                        "end_gold": record["end_gold"],
                        "start_logits": [],
                        "end_logits": [],
                    }
                train_dynamics[guid]["start_logits"].append(
                    record[f"start_logits_epoch_{epoch_num}"]
                )
                train_dynamics[guid]["end_logits"].append(
                    record[f"end_logits_epoch_{epoch_num}"]
                )
