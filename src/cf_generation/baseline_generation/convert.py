...
def get_arg_parser():
    ...
    parser.add_argument(
        "--block_records_path", type=str, required=True, help="Block records path."
    )
    # ./data/cc_news_pretrained/embedder/encoded/encoded.ckpt
    parser.add_argument(
        "--block_emb_path", type=str, required=True, help="Block embeddings path."
    )
    ...
    pretrained_group.add_argument(
        "--embedder_path",
        type=str,
        default=r"./data/cc_news_pretrained/embedder/variables/variables",
        help="Pretrained embedder path.",
    )
    pretrained_group.add_argument(
        "--bert_path",
        type=str,
        default=r"./data/cc_news_pretrained/bert/variables/variables",
        help="Pretrained bert path.",
    )

    finetuned_group = parser.add_argument_group("finetuned conversion")

    finetuned_group.add_argument(
        "--checkpoint_path",
        type=str,
        default=r"./data/orqa_nq_model_from_realm/export/best_default/checkpoint/model.ckpt-300000",
    ...
    parser.add_argument(
        "--from_pretrained",
        action="store_true",
        help="Whether to convert from a pretrained checkpoint or a fine-tuned checkpoint.",
    )
