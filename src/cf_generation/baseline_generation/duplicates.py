...
def remove_duplicates():
    examples = []
    with jsonlines.open(
        BASE_PATH + "src/data/squad/squad_counterfactuals_noise_min_filtered.jsonl"
    ) as reader:
    ...
    save_to_disk(
        filtered,
        BASE_PATH
        + "src/data/squad/squad_counterfactuals_noise_min_filtered_final.jsonl",
    )
...
def remove_duplicate_counterfactuals():
    ...
    with jsonlines.open(
        BASE_PATH
        + "src/data/squad/t5_squad_counterfactuals/rag_counterfactuals_complete_noise_min_filtered_final_1.jsonl"
    ) as reader:
        for example in tqdm(reader):
            examples.append(example)
    filtered = []
    for example in tqdm(
        examples, total=len(examples), desc="Removing duplicate cfs ... "
    ):
    ...
    save_to_disk(
        filtered,
        BASE_PATH
        + "src/data/squad/t5_squad_counterfactuals/rag_counterfactuals_complete_noise_min_filtered_final_dedup_1.jsonl",
    )
