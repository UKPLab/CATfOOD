...
class RealmReader(RealmPreTrainedModel):
    ...
    def forward(
        ...
        # []
        starts : torch.LongTensor = torch.tensor([], device=reader_logits.device)
        ends : torch.LongTensor = torch.tensor([], device=reader_logits.device)

        for logits in reader_logits:
            logits = torch.unsqueeze(logits, 0)
            predicted_block_index = torch.argmax(torch.max(logits, dim=1).values)
            predicted_candidate = torch.argmax(torch.max(logits, dim=0).values)
            ...
            starts = torch.cat((starts, predicted_start), dim=0)
            ends = torch.cat((ends, predicted_end), dim=0)

        # get the context to retrieve the answer
        ...
        if not return_dict:
            output = (predicted_block_index, predicted_candidate, starts, ends) + outputs[2:]
            return (
...
class RealmForOpenQA(RealmPreTrainedModel):
    ...
    #     tokenizer = self.config.tokenizer.add_special_tokens(
    #     {"additional_special_tokens": ["<hl>"]}
    # )

        batch_answer_ids : list = []
        sequence_length = 128
        for idx in range(self.config.reader_beam_size):
            predicted_block = concat_inputs.input_ids[idx]
            start = reader_output.start_pos.detach().cpu().numpy()[idx]
            end = reader_output.end_pos.detach().cpu().numpy()[idx]
            predicted_answer_ids = predicted_block[int(start) : int(end) + 1]
            pad = torch.zeros(sequence_length - len(predicted_answer_ids), dtype=torch.long, device=self.reader.device)
            batch_answer_ids.append(torch.cat((predicted_answer_ids, pad), dim=0))
            # print(predicted_block)
        batch_answer_ids = torch.stack(batch_answer_ids)

        if not return_dict:
            return reader_output, batch_answer_ids

        return RealmForOpenQAOutput(
            reader_output=reader_output,
            retrieved_block_ids=retrieved_block_ids,
            input_ids=concat_inputs.input_ids,
            predicted_answer_ids=batch_answer_ids,
        )
